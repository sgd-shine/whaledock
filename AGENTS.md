...
本仓库是 **Harness Desktop**：非官方 DeepSeek Harness macOS 桌面客户端（Electron 43）。完整设计文档在 `docs/开发方案.md`，**动手前先读它**，尤其第 7 节（当前状态）和第 8 节（工程约束与验收清单）。

## 项目状态一句话

代码已完整实现并在 Linux 容器通过 12 项冒烟测试 + 无头启动测试（SMOKE_OK），但**从未在真实 macOS 上跑过**。当前阶段的工作是真机验证与修复，不是重写。

## 常用命令

```bash
npm install        # 安装依赖（仓库带 package-lock.json）
npm run smoke      # 纯 Node 冒烟测试，12 项，必须 ALL PASS —— 每次改动后都要跑
npm start          # 启动 App（真机验证入口）
npm run dist       # 打包 .dmg / .zip 到 release/
```

调试技巧：`HARNESS_SMOKE=1 npm start` 是无头自测模式（窗口就绪打印 SMOKE_OK 自动退出）；后端日志在 `~/Library/Application Support/Harness Desktop/logs/`；配置文件在同目录 `config.json`，改 `command` 字段可强制指定后端启动命令。手动验证"接入已有服务"路径：先在终端 `npx -y @deepseek-ai/dsh web`，再 `npm start`。
...
4. 对 dsh 的假设（端口、子命令、启动行为）只允许写在 `lib/backend.js` 的命令解析和 `lib/config.js` 的默认值里，不得散落在业务逻辑中写死。dsh 目前是 rc 版本（0.1.0-rc.x），行为可能变化。
5. 保持 MIT 协议、"Unofficial / 非官方"定位表述、不使用 DeepSeek 商标素材。
