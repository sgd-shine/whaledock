...
v0.1.1 已发布，v0.2.0 正在正式发布：Windows x64（NSIS 安装器 + 便携版）、免装 Node 内置引擎、Intel/Apple Silicon Mac 分包、设置窗（含开机自启/启动最小化）与自研更新器。macOS 本地验证、三平台 CI、Windows/Intel 产物与人工验收必须分开报告；尤其不能把 Windows CI 构建成功写成 dsh Windows 真机已通过。2026-08-15 SGD 已授权本轮由代理直接发布 v0.2–v0.5；该授权不包含签名、公证、付费或放宽硬约束。

## 常用命令

```bash
npm install               # 安装源码依赖（仓库带 package-lock.json）
npm run smoke             # 纯 Node 跨平台冒烟测试，v0.2 当前 34 项；必须 ALL PASS
...
- 发布：本轮 v0.2–v0.5 由代理按 2026-08-15 授权创建正式 tag/Release；仍不得使用证书、签名、公证、付费服务，也不得绕过精确审批门。
