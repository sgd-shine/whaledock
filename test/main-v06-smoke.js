...
async function main06() {
  ...
  await test('外壳 preload 只暴露固定通道，不泄露 ipcRenderer，也不 require fs', async () => {
    ...
    assert.deepEqual([...new Set(invoked)], [
      'shell:action', 'shell:cockpit-theme', 'shell:cockpit-view', 'shell:delivery-ack',
      'shell:delivery-open', 'shell:get', 'shell:install',
  ...
  await test('外壳 IPC 全部走三重校验，且通道集合与 preload 完全对上', async () => {
    ...
    assert.deepEqual(handled, [
      'shell:action', 'shell:cockpit-theme', 'shell:cockpit-view', 'shell:delivery-ack',
      'shell:delivery-open', 'shell:get', 'shell:install', 'shell:onboarding-seen',
