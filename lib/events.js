...
const MAX_DELIVERY_TRACKER_ENTRIES = 1_000;
const DELIVERY_TRACKER_TTL_MS = 24 * 60 * 60 * 1000;
...
const DELIVERY_ADMISSION_STATES = new Set(['accepted', 'rejected', 'unknown']);
const DELIVERY_TERMINAL_STATES = new Set(['completed', 'error']);
const DELIVERY_PLACEMENT_RE = /^[a-z0-9][a-z0-9._-]{0,63}$/;
const DELIVERY_REASON_RE = /^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/;
...
function cloneDeliveryTracker(tracker) {
  const copy = new Map();
  for (const [deliveryKey, entry] of tracker) copy.set(deliveryKey, { ...entry });
  return copy;
}
...
function createEventService(options = {}) {
  ...
  // 投递回执只属于当前进程：key 和 session 都是 HMAC，不写入 schema 1。
  let deliveryTracker = new Map();
  let taskDeliveryBindings = new Map();
  let committedDeliveryTracker = deliveryTracker;
  let availability = {
    state: 'probing',
  ...
  function deliveryKeyFor(deliveryRef) {
    return hmac('delivery', normalizeRef(deliveryRef, 'deliveryRef'));
  }

  function normalizeDeliveryReason(value) {
    if (value === undefined || value === null || value === '') return null;
    if (typeof value !== 'string' || !DELIVERY_REASON_RE.test(value)) {
      throw eventError('delivery reason 必须是短安全代码');
    }
    return value;
  }

  function normalizeDeliveryPlacement(value) {
    if (Number.isSafeInteger(value) && value >= 0) return value;
    if (typeof value === 'string' && DELIVERY_PLACEMENT_RE.test(value)) return value;
    return null;
  }

  function removeDelivery(deliveryKey) {
    const entry = deliveryTracker.get(deliveryKey);
    if (!entry) return;
    if (entry.taskKey && taskDeliveryBindings.get(entry.taskKey) === deliveryKey) {
      taskDeliveryBindings.delete(entry.taskKey);
    }
    deliveryTracker.delete(deliveryKey);
  }

  function pruneDeliveries() {
    const observedNow = new Date(now()).getTime();
    for (const [deliveryKey, entry] of deliveryTracker) {
      if (observedNow - entry.touchedAtMs > DELIVERY_TRACKER_TTL_MS) {
        removeDelivery(deliveryKey);
      }
    }
    if (deliveryTracker.size <= MAX_DELIVERY_TRACKER_ENTRIES) return;
    const ordered = [...deliveryTracker.entries()].sort((left, right) => {
      const leftTerminal = DELIVERY_TERMINAL_STATES.has(left[1].state) ? 0 : 1;
      const rightTerminal = DELIVERY_TERMINAL_STATES.has(right[1].state) ? 0 : 1;
      return leftTerminal - rightTerminal || left[1].touchedAtMs - right[1].touchedAtMs;
    });
    for (const [deliveryKey] of ordered.slice(
      0, deliveryTracker.size - MAX_DELIVERY_TRACKER_ENTRIES
    )) removeDelivery(deliveryKey);
  }

  function touchDelivery(entry, at) {
    entry.updatedAt = at;
    entry.touchedAtMs = new Date(now()).getTime();
  }

  function deliveryUnknown(reason) {
    return {
      state: 'unknown',
      result: null,
      reason,
  ...
  function deliveryProjectionReason(entry) {
    if (entry.admission === 'unknown' && entry.state === 'submitting') {
      return entry.reason || 'admission-unknown';
    }
    if (committedAvailability.state !== 'live') return 'events-not-live';
    if (committedAvailability.detail === 'history-gap') return 'history-gap';
    const session = committedState.sessions[entry.sessionKey];
    if (!session) return 'session-unobserved';
    if (session.notificationFloorSeq === null) return 'session-coverage-partial';
    if (session.coverageGap) return 'session-history-gap';
    const pending = committedBuffers.get(entry.sessionKey);
    if (pending && pending.size > 0) return 'session-sequence-gap';
    return null;
  }

  function projectDelivery(entry) {
    const terminal = DELIVERY_TERMINAL_STATES.has(entry.state);
    const projectionReason = terminal ? null : deliveryProjectionReason(entry);
    return {
      state: projectionReason ? 'unknown' : entry.state,
      result: terminal ? entry.result : null,
      reason: projectionReason || entry.reason,
  ...
  function deliverySnapshot(deliveryRef) {
    assertOpen();
    const entry = committedDeliveryTracker.get(deliveryKeyFor(deliveryRef));
    if (entry && new Date(now()).getTime() - entry.touchedAtMs > DELIVERY_TRACKER_TTL_MS) {
      return deliveryUnknown('not-tracked');
    }
    return entry ? projectDelivery(entry) : deliveryUnknown('not-tracked');
  }

  function registerDelivery(input) {
    assertOpen();
    if (!isPlainObject(input)) throw eventError('delivery 注册参数必须是对象');
    const deliveryKey = deliveryKeyFor(input.deliveryRef);
    const sessionKey = sessionKeyFor(input.sessionRef);
    const at = safeIso(input.at, new Date(now()));
    pruneDeliveries();
    const existing = deliveryTracker.get(deliveryKey);
    if (existing) {
      if (existing.sessionKey !== sessionKey) {
        throw eventError('deliveryRef 不得改绑 session', 'ERR_EVENT_DELIVERY_SESSION');
      }
      existing.touchedAtMs = new Date(now()).getTime();
      return projectDelivery(existing);
    }
    deliveryTracker.set(deliveryKey, {
      sessionKey,
      state: 'submitting',
      admission: null,
      result: null,
      ...
      finishedAt: null,
      updatedAt: at,
      touchedAtMs: new Date(now()).getTime(),
      placement: null,
      taskKey: null,
      turn: null
    });
    pruneDeliveries();
    const registered = deliveryTracker.get(deliveryKey);
    return registered ? projectDelivery(registered) : deliveryUnknown('tracker-capacity');
  }

  function settleDeliveryAdmission(input) {
    assertOpen();
    if (!isPlainObject(input)) throw eventError('delivery 受理参数必须是对象');
    const stateValue = input.state;
    if (!DELIVERY_ADMISSION_STATES.has(stateValue)) {
      throw eventError('delivery admission state 不受支持');
    }
    const deliveryKey = deliveryKeyFor(input.deliveryRef);
    const at = safeIso(input.at, new Date(now()));
    normalizeDeliveryReason(input.reason);
    pruneDeliveries();
    const entry = deliveryTracker.get(deliveryKey);
    if (!entry) return deliveryUnknown('not-tracked');
    if (DELIVERY_TERMINAL_STATES.has(entry.state)) return projectDelivery(entry);

    if (stateValue === 'accepted') {
      if (entry.state === 'submitting') {
        entry.state = 'queued';
        entry.queuedAt = entry.queuedAt || at;
      }
      entry.admission = 'accepted';
      entry.reason = null;
      touchDelivery(entry, at);
    } else if (stateValue === 'rejected') {
      // 队列或 turn 已经证明接收时，迟到的拒绝不能覆盖更强证据。
      if (entry.state === 'submitting') {
        entry.state = 'error';
        entry.admission = 'rejected';
        entry.result = 'rejected';
        entry.reason = 'admission-rejected';
        entry.finishedAt = at;
        touchDelivery(entry, at);
      }
    } else if (entry.admission === null || entry.admission === 'unknown') {
      entry.admission = 'unknown';
      entry.reason = 'admission-unknown';
      touchDelivery(entry, at);
    }
    return projectDelivery(entry);
  }
  ...
  function prepareDeliveryQueueEvent(input) {
    if (!isPlainObject(input) || input.kind !== 'queue-snapshot') {
      throw eventError('queue snapshot 必须是中性事件');
    }
    if (input.seq !== undefined) throw eventError('queue snapshot 不得带 seq');
    if (!Array.isArray(input.items) || input.items.length > 10_000) {
      throw eventError('queue snapshot items 必须是有界数组');
    }
    ...
    for (const item of input.items) {
      ...
      let deliveryKey;
      try {
        deliveryKey = deliveryKeyFor(item.deliveryRef);
      } catch (_error) {
        continue;
      }
      ...
      items.push({
        deliveryKey,
        placement: normalizeDeliveryPlacement(item.placement)
      });
    }
    return {
      kind: 'queue-snapshot',
      sessionKey,
      sessionRef,
      at: safeIso(input.observedAt === undefined ? input.at : input.observedAt, new Date(now())),
      items
    };
  }

  function applyDeliveryQueuePrepared(event) {
    pruneDeliveries();
    let observed = 0;
    for (const item of event.items) {
      const entry = deliveryTracker.get(item.deliveryKey);
      if (!entry || entry.sessionKey !== event.sessionKey
          || DELIVERY_TERMINAL_STATES.has(entry.state)) continue;
      if (entry.state === 'submitting') {
        entry.state = 'queued';
        entry.queuedAt = entry.queuedAt || event.at;
      }
      // queue 出现是比 admission unknown 更强的本地接收证据。
      if (entry.state === 'queued') {
        entry.admission = 'accepted';
        entry.reason = null;
        entry.placement = item.placement;
        touchDelivery(entry, event.at);
      }
      observed += 1;
    }
    return observed;
  ...
  function observeDeliveryQueue(input) {
    assertOpen();
    const event = prepareDeliveryQueueEvent(input);
    return { observed: applyDeliveryQueuePrepared(event) };
  }

  function updateSessionDeliveryWaiting(sessionKey) {
    const session = state.sessions[sessionKey];
    const hasPending = session && (
      Object.keys(session.pendingApprovals).length > 0
      || Object.keys(session.pendingQuestions).length > 0
    );
    const active = [];
    for (const entry of deliveryTracker.values()) {
      if (entry.sessionKey === sessionKey && entry.taskKey
          && (entry.state === 'running' || entry.state === 'waiting')) active.push(entry);
    }
    const unique = hasPending && active.length === 1 ? active[0] : null;
    const at = new Date(now()).toISOString();
    for (const entry of active) {
      const next = entry === unique ? 'waiting' : 'running';
      if (entry.state !== next) {
        entry.state = next;
        touchDelivery(entry, at);
      }
  ...
  function bindDeliveryToTask(event, turn) {
    if (!event.deliveryKey || event.role !== 'user') return;
    const entry = deliveryTracker.get(event.deliveryKey);
    if (!entry || entry.sessionKey !== event.sessionKey
        || DELIVERY_TERMINAL_STATES.has(entry.state)) return;
    const occupied = taskDeliveryBindings.get(event.taskKey);
    if ((occupied && occupied !== event.deliveryKey)
        || (entry.taskKey && entry.taskKey !== event.taskKey)) return;
    taskDeliveryBindings.set(event.taskKey, event.deliveryKey);
    entry.taskKey = event.taskKey;
    entry.turn = event.turn;
    entry.admission = 'accepted';
    entry.reason = null;
    entry.state = 'running';
    entry.startedAt = turn.startedAt || event.at;
    touchDelivery(entry, event.at);
    updateSessionDeliveryWaiting(event.sessionKey);
  }

  function updateDeliveryStart(event, turn) {
    const deliveryKey = taskDeliveryBindings.get(event.taskKey);
    const entry = deliveryKey ? deliveryTracker.get(deliveryKey) : null;
    if (!entry || DELIVERY_TERMINAL_STATES.has(entry.state)) return;
    entry.startedAt = turn.startedAt;
    touchDelivery(entry, event.at);
  }

  function finishDelivery(event, result) {
    const deliveryKey = taskDeliveryBindings.get(event.taskKey);
    const entry = deliveryKey ? deliveryTracker.get(deliveryKey) : null;
    if (!entry || DELIVERY_TERMINAL_STATES.has(entry.state)) return;
    entry.state = result === 'completed' ? 'completed' : 'error';
    entry.result = result;
    entry.reason = result === 'completed' ? null : `terminal-${result}`;
  ...
  async function transaction(work, options = {}) {
    return enqueue(async () => {
      ...
      const deliveryTrackerBefore = deliveryTracker;
      const taskDeliveryBindingsBefore = taskDeliveryBindings;
      deliveryTracker = cloneDeliveryTracker(deliveryTrackerBefore);
      taskDeliveryBindings = new Map(taskDeliveryBindingsBefore);
      ...
      try {
        ...
        committedDeliveryTracker = deliveryTracker;
        ...
        deliveryTracker = deliveryTrackerBefore;
        taskDeliveryBindings = taskDeliveryBindingsBefore;
  ...
  function prepareEvent(input, suppressNotifications) {
    ...
    if (input.kind === 'queue-snapshot') {
      return {
        ...prepareDeliveryQueueEvent(input),
    ...
    if (input.kind === 'turn-start' || input.kind === 'message'
        || input.kind === 'turn-terminal') {
    ...
    if (input.kind === 'turn-start') return prepared;
    ...
    if (input.kind === 'message') {
      ...
      if (input.deliveryRef !== undefined && input.deliveryRef !== null) {
        prepared.deliveryKey = deliveryKeyFor(input.deliveryRef);
      }
  ...
  function applyPrepared(event, effects) {
    ...
    if (event.kind === 'queue-snapshot') {
      applyDeliveryQueuePrepared(event);
      return false;
    }
    ...
    if (event.kind.startsWith('approval-') || event.kind.startsWith('question-')) {
      const changed = applyWaiting(event, session, effects);
      updateSessionDeliveryWaiting(event.sessionKey);
      return changed;
    }
    if (event.kind === 'turn-start') {
      const turn = getTurn(session, event);
      // message 只能提供首次观测时间；显式 turn-start 才是真实开始时间。
      turn.startSeq = event.seq;
      turn.startedAt = event.at;
      updateDeliveryStart(event, turn);
      return true;
    }
  ...
  async function disconnect(generation) {
    return transaction(async () => {
      ...
      for (const [sessionKey, session] of Object.entries(state.sessions)) {
        ...
        updateSessionDeliveryWaiting(sessionKey);
      }
  ...
  return {
    ...
    ingestMany,
    registerDelivery,
    settleDeliveryAdmission,
    observeDeliveryQueue,
    deliverySnapshot,
    registerSession,
    getCursor,
    beginConnection,
