...
function resolveWorkspaceFile(root, relative, options = {}) {
  ...
  let canonicalRoot;
  try {
    canonicalRoot = realpathWith(fsImpl, path.resolve(root));
  } catch (error) {
    if (error && error.code === 'ERR_FS_IMPL') throw error;
    throw cockpitError('工作区根目录不可读', 'ERR_ROOT_UNREADABLE');
  }
  const candidate = path.join(path.resolve(root), ...clean.split('/'));
  let stat;
  try {
  ...
  if (stat.isSymbolicLink()) throw cockpitError('拒绝读取文件软链接', 'ERR_PATH_SYMLINK');
  if (!stat.isFile()) throw cockpitError('目标不是普通文件', 'ERR_PATH_NOT_FILE');
  if (!Number.isFinite(stat.size) || stat.size < 0 || stat.size > LIMITS.maxFileBytes) {
    throw cockpitError('文件超过 512 KiB', 'ERR_FILE_TOO_LARGE');
  }

  let canonicalFile;
  try {
    canonicalFile = realpathWith(fsImpl, candidate);
  } catch (_error) {
...
function readDocument(root, relative, options = {}) {
  ...
  const target = resolveWorkspaceFile(root, relativePath, { fsImpl });
  let value;
  try {
    value = fsImpl.readFileSync(target);
  } catch (_error) {
...
function scanWorkspace(root, options = {}) {
  ...
  try {
    ...
    const stat = fsImpl.statSync(requested);
    if (!stat.isDirectory()) throw new Error('not-directory');
    scanRoot = realpathWith(fsImpl, requested);
  } catch (_error) {
    throw cockpitError('工作区根目录不存在或不可读', 'ERR_ROOT_UNREADABLE');
  }
  const relativeFiles = [];
  const issues = [];
  ...
  for (const location of APPROVED_LOCATIONS) visitDirectory(location.relative);
  ...
  return { items, stageCounts, today, issues, truncated };
}
