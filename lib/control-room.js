...
function safeRecent(value) {
  if (!plainRecord(value) || !['user', 'assistant'].includes(value.role)
      || typeof value.text !== 'string' || !value.text.trim()
      || value.text.length > LIMITS.maxRecentChars
      || /[\u0000-\u001f\u007f]/.test(value.text)
      || !Number.isSafeInteger(value.updatedAt) || value.updatedAt < 0) return null;
  return Object.freeze({ role: value.role, text: value.text.trim(), updatedAt: value.updatedAt });
}
...
function buildCards(input) {
  ...
  const cards = projects.map((project) => {
    ...
    return {
      ...
      recent: sid && snapshot.byId[sid] ? snapshot.byId[sid].recent : null
    };
  });
