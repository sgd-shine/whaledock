...
const SKIN_COLOR_RE = /^#[a-f0-9]{6}$/;
const SKIN_COLOR_KEYS = Object.freeze([
  'background', 'surface', 'border', 'primary', 'accent', 'text', 'textMuted'
]);
const ADOPTED_MODES = new Set(['existing', 'relinked', 'manifest', 'new']);
...
function skinSurface(value) {
  if (value === null || value === undefined) return null;
  if (!exactKeys(value, ['base', 'colors']) || !['dark', 'light'].includes(value.base)
      || !exactKeys(value.colors, SKIN_COLOR_KEYS)
      || SKIN_COLOR_KEYS.some((key) => (
        typeof value.colors[key] !== 'string' || !SKIN_COLOR_RE.test(value.colors[key])
      ))) return null;
  return deepFreeze({
    base: value.base,
    colors: Object.fromEntries(SKIN_COLOR_KEYS.map((key) => [key, value.colors[key]]))
  });
}
...
function validateEmpty(input) {
  if (!exactKeys(input, [])) invalid('项目操作参数无效');
  return Object.freeze({});
}

function validateDetach(input) {
  if (!exactKeys(input, ['projectId', 'window', 'tabId'])
      || !Number.isSafeInteger(input.window) || input.window < 1 || input.window > 16
      || typeof input.tabId !== 'string' || !input.tabId || input.tabId.length > 128
      || CONTROL_RE.test(input.tabId)) invalid('分离窗参数无效');
  return Object.freeze({
    projectId: projectId(input.projectId), window: input.window, tabId: input.tabId
  });
}
...
function detailFromStore(value, hasFolderOverride, actionsValue, previewsValue = [],
    maxBytes = MAX_PROJECT_DETAIL_BYTES, skinValue = null) {
  ...
  const skin = skinSurface(skinValue);
  const build = () => ({
    ...summary,
...
function detailFromResult(value) {
  ...
  ], ['skin'])) throw operationError('ERR_PROJECT_RUNTIME', '项目详情结果无效');
  ...
  return Object.freeze({
    ...
    ...(Object.prototype.hasOwnProperty.call(value, 'skin')
      ? { skin: skinSurface(value.skin) } : {})
  });
}
...
function cardResult(value) {
  if (!isPlainObject(value) || !STATUSES.has(value.status)
      ...
      || !(value.recent === null || (isPlainObject(value.recent)
        && exactKeys(value.recent, ['role', 'text', 'updatedAt'])
        && ['user', 'assistant'].includes(value.recent.role)
        && typeof value.recent.text === 'string' && value.recent.text.trim()
        && value.recent.text.length <= 160 && !CONTROL_RE.test(value.recent.text)
        && Number.isSafeInteger(value.recent.updatedAt) && value.recent.updatedAt >= 0))) {
  ...
  return Object.freeze({
    ...
    recent: value.recent === null ? null : Object.freeze({ ...value.recent })
  });
}

function redactAdopted(value) {
  if (!exactKeys(value, ['kind', 'revision', 'adopted', 'project'])
      || value.kind !== 'adopted' || !ADOPTED_MODES.has(value.adopted)
      || !Number.isSafeInteger(value.revision) || value.revision < 0) {
    throw operationError('ERR_PROJECT_RUNTIME', '项目认领结果无效');
  }
  return Object.freeze({
    kind: 'adopted', revision: value.revision, adopted: value.adopted,
    project: summaryFromResult(value.project)
  });
}

function redactSidecar(value) {
  if (!exactKeys(value, ['kind', 'revision', 'projectId', 'written'])
      || value.kind !== 'sidecar' || value.written !== true
      || !Number.isSafeInteger(value.revision) || value.revision < 0) {
    throw operationError('ERR_PROJECT_RUNTIME', '项目旁车结果无效');
  }
  return Object.freeze({
    kind: 'sidecar', revision: value.revision,
    projectId: projectId(value.projectId), written: true
  });
}

function redactDetached(value) {
  if (!exactKeys(value, ['kind', 'projectId', 'window', 'tabId'])
      || value.kind !== 'detached' || !Number.isSafeInteger(value.window)
      || value.window < 1 || value.window > 16 || typeof value.tabId !== 'string'
      || !value.tabId || value.tabId.length > 128 || CONTROL_RE.test(value.tabId)) {
    throw operationError('ERR_PROJECT_RUNTIME', '分离窗结果无效');
  }
  return Object.freeze({ ...value, projectId: projectId(value.projectId) });
}
...
function publicErrorCode(error) {
  ...
  if (code === 'ERR_PROJECT_IDENTITY_CONFLICT') return 'project-identity-conflict';
...
function createProjectOperations(options = {}) {
  if (!isPlainObject(options)
      || Object.keys(options).some((key) => ![
        ...
        'onProjectOpenOutcomeUnknown', 'bootstrapTicketFor', 'skinForTemplate', 'onDetach'
      ].includes(key))) {
  ...
  const skinForTemplate = options.skinForTemplate === undefined
    ? () => null : options.skinForTemplate;
  const onDetach = options.onDetach === undefined ? () => {} : options.onDetach;
  const storeMethods = [
  ...
  if (!projectStore || storeMethods.some((name) => typeof projectStore[name] !== 'function')
      ...
      || typeof skinForTemplate !== 'function' || typeof onDetach !== 'function') {
  ...
  return Object.freeze({
    ...
    'projects.adopt': descriptor(validateEmpty, async ({ context }) => {
      assertCurrent(context);
      const folder = await chooseFolder();
      assertCurrent(context);
      if (folder === null || folder === undefined) {
        throw operationError('ERR_PROJECT_CANCELLED', '用户取消认领项目文件夹');
      }
      if (typeof folder !== 'string' || !folder.trim() || CONTROL_RE.test(folder)) {
        throw operationError('ERR_PROJECT_FOLDER', '目录选择器返回无效项目文件夹');
      }
      if (typeof projectStore.adoptFolder !== 'function') {
        throw operationError('ERR_PROJECT_RUNTIME', '项目认领能力不可用');
      }
      const adopted = projectStore.adoptFolder(folder);
      assertCurrent(context);
      if (!isPlainObject(adopted) || !ADOPTED_MODES.has(adopted.adopted)) {
        throw operationError('ERR_PROJECT_RUNTIME', '项目认领结果无效');
      }
      return Object.freeze({
        kind: 'adopted', revision: revisionOf(projectStore), adopted: adopted.adopted,
        project: summaryFromStore(adopted.project)
      });
    }, redactAdopted),
    ...
    'projects.sidecar': descriptor(validateProjectOnly, async ({ input, context }) => {
      assertCurrent(context);
      const current = projectStore.get(input.projectId);
      if (!current) throw operationError('ERR_PROJECT_NOT_FOUND', '项目不存在');
      if (typeof projectStore.writeManifest !== 'function') {
        throw operationError('ERR_PROJECT_RUNTIME', '项目旁车能力不可用');
      }
      projectStore.writeManifest(input.projectId);
      return Object.freeze({
        kind: 'sidecar', revision: revisionOf(projectStore),
        projectId: input.projectId, written: true
      });
    }, redactSidecar),
    ...
    'projects.open': descriptor(validateOpen, async ({ input, binding, context }) => {
      ...
      if (input.phase === 'prepare') {
        ...
        assertCurrent(context);
        let skin = null;
        try { skin = await skinForTemplate(current.templateId); }
        catch (_error) { skin = null; }
        assertCurrent(context);
    ...
    'projects.detach': descriptor(validateDetach, async ({ input, context }) => {
      assertCurrent(context);
      const current = projectStore.get(input.projectId);
      if (!current) throw operationError('ERR_PROJECT_NOT_FOUND', '项目不存在');
      const pane = paneStateFromStore(current.paneState);
      const targetWindow = pane && pane.windows.find((item) => item.window === input.window);
      const tab = targetWindow && targetWindow.tabs.find((item) => item.id === input.tabId);
      if (!tab) invalid('分离窗目标已失效');
      let detachedHandle = null;
      try {
        detachedHandle = await Promise.resolve(onDetach(Object.freeze({
          project: Object.freeze({
            projectId: input.projectId, name: safeName(current.name), icon: safeIcon(current.icon)
          }),
          window: input.window,
          tab: deepFreeze({ ...tab })
        })));
        ...
        // 原生窗口打开后，页面 fence 可能在 await 期间变化；此时撤销刚创建的窗口，
        // 不能返回 operation-stale 却留下显示错误项目的幽灵窗口。
        if (detachedHandle && typeof detachedHandle.destroy === 'function') {
          try { await Promise.resolve(detachedHandle.destroy()); }
          catch (_cleanupError) { /* best effort；原错误仍是唯一公开结果 */ }
        }
        throw error;
      }
      return Object.freeze({ kind: 'detached', ...input });
    }, redactDetached),
