...
const MAX_RECORD_RETAKES = 1_000_000_000;
...
function planWriteback(summary) {
  ...
  if (!Number.isSafeInteger(totalRetakes) || totalRetakes > MAX_RECORD_RETAKES) {
    throw shootingError('ERR_SHOOTING_SUMMARY', '收工摘要重来次数超出上限');
  }
...
function recordError(message) {
  return shootingError('ERR_SHOOTING_RECORD', message);
}

function canonicalJsonString(raw, label) {
  let value;
  try { value = JSON.parse(raw); }
  catch (_error) { throw recordError(`拍摄记录的${label}不是合法字符串`); }
  if (typeof value !== 'string' || JSON.stringify(value) !== raw) {
    throw recordError(`拍摄记录的${label}不是规范字符串`);
  }
  return value;
}
...
function recordInteger(raw, label, maximum) {
  if (!/^(?:0|[1-9]\d{0,9})$/.test(raw)) {
    throw recordError(`拍摄记录的${label}不是规范整数`);
  }
  const value = Number(raw);
  if (!Number.isSafeInteger(value) || value < 0 || value > maximum) {
    throw recordError(`拍摄记录的${label}超出上限`);
  }
  return value;
}

// 只识别 planWriteback 生成的收工记录头和摘要。记录仍是用户可编辑的本地文件，
// 因此调用方只能把结果表述为“记录中写着”，不能冒充视频或平台回读。
function parseOwnedRecord(text) {
  if (typeof text !== 'string' || utf8Bytes(text) > LIMITS.sourceBytes
      || hasUnsafeControls(text)) {
    throw recordError('拍摄记录不是受限 UTF-8 文本');
  }
  const normalized = text.replace(/\r\n?/g, '\n');
  const lines = normalized.split('\n');
  if (lines[0] !== OWNERSHIP_MARKER
      || lines[1] !== '---'
      || lines[2] !== 'whaledock-schema: 1'
      || lines[3] !== 'kind: shooting-session'
      || lines[7] !== '---'
      || lines[8] !== '') {
    throw recordError('拍摄记录所有权或结构不匹配');
  }
  const sessionMatch = /^session-id: (.+)$/.exec(lines[4] || '');
  const sourceMatch = /^source: (.+)$/.exec(lines[5] || '');
  const hashMatch = /^source-sha256: (.+)$/.exec(lines[6] || '');
  if (!sessionMatch || !sourceMatch || !hashMatch) {
    throw recordError('拍摄记录身份字段不完整');
  }
  const sessionId = canonicalJsonString(sessionMatch[1], 'session-id');
  const sourceRelativePath = canonicalJsonString(sourceMatch[1], 'source');
  const rawSourceHash = canonicalJsonString(hashMatch[1], 'source-sha256');
  try {
    validateSessionId(sessionId);
    validateRelativePath(sourceRelativePath);
    validateSourceHash(rawSourceHash);
  } catch (_error) {
    throw recordError('拍摄记录身份字段无效');
  }
  if (!sourceRelativePath.startsWith('03_口播稿/')
      || rawSourceHash !== rawSourceHash.toLowerCase()) {
    throw recordError('拍摄记录来源不是受控口播稿');
  }

  const titleMatch = /^# 拍摄收工记录 · (.+)$/.exec(lines[9] || '');
  const confirmedMatch = /^- 已确认：([0-9]+)\/([0-9]+)$/.exec(lines[11] || '');
  const missingMatch = /^- 未确认或缺拍：([0-9]+)$/.exec(lines[12] || '');
  const retakesMatch = /^- 重来次数：([0-9]+)$/.exec(lines[13] || '');
  if (!titleMatch || lines[10] !== '' || !confirmedMatch || !missingMatch
      || !retakesMatch || lines[14] !== '') {
    throw recordError('拍摄记录摘要结构不完整');
  }
  const rawTitle = titleMatch[1];
  const decodedTitle = unescapeMarkdownCell(rawTitle);
  if (/[\u0000-\u001f\u007f]/.test(decodedTitle)) {
    throw recordError('拍摄记录标题含控制字符');
  }
  let title;
  try { title = validateDocumentTitle(decodedTitle); }
  catch (_error) { throw recordError('拍摄记录标题无效'); }
  if (title !== decodedTitle || markdownCell(title) !== rawTitle) {
    throw recordError('拍摄记录标题不是规范写法');
  }
  const confirmedCount = recordInteger(confirmedMatch[1], '已确认数量', LIMITS.shots);
  const totalShots = recordInteger(confirmedMatch[2], '镜头总数', LIMITS.shots);
  const missingCount = recordInteger(missingMatch[1], '缺拍数量', LIMITS.shots);
  const retakeCount = recordInteger(retakesMatch[1], '重来次数', MAX_RECORD_RETAKES);
  if (totalShots < 1 || confirmedCount > totalShots
      || missingCount > totalShots || confirmedCount + missingCount !== totalShots) {
    throw recordError('拍摄记录摘要计数不一致');
  }
  return Object.freeze({
    schemaVersion: 1,
    sessionId,
