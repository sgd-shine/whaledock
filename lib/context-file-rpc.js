...
function normalizeBinding(value) {
  if (!exactKeys(value, BINDING_KEYS)
    ...
    throw rpcError('ERR_BINDING_INVALID', '文件 RPC 工作区绑定无效');
  }
...
function sameBinding(left, right) {
  return Boolean(left && right)
    && BINDING_KEYS.slice(0, -1).every((key) => left[key] === right[key])
...
function normalizeDescriptor(value) {
  ...
  if (!['validate', 'handle', 'redact'].every((key) => typeof value[key] === 'function')
      ...
      || keys.some((key) => !['validate', 'handle', 'redact', 'errorCode'].includes(key))) {
...
function createContextFileRpcBroker(options = {}) {
  ...
  const registerOperation = (name, descriptor) => {
    ...
    operations.set(operation, normalizeDescriptor(descriptor));
    return operation;
  };
  ...
  const enqueue = (raw) => {
    ...
    if (jsonBytes(candidate) > limits.maxInputBytes) {
    ...
    if (jsonBytes(input) > limits.maxInputBytes) {
      throw rpcError('ERR_INPUT_TOO_LARGE', 'operation 输入超过上限');
    }
