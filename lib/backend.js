...
function contextPocPlan(command, info = runtimeInfo, runtime = {}) {
  ...
  if (!sourceRoot || !userDataPath || !pathImpl.isAbsolute(sourceRoot)
      || !pathImpl.isAbsolute(userDataPath) || sourceRoot.includes('\0')
...
function contextPocRelativeTarget(relative) {
  ...
  return path.posix.join(
    'home', 'profiles', 'web', 'node_modules', '@whaledock',
...
function contextPocReadAssets(plan, runtime = {}) {
  ...
  exactEntries(plan.sourceRoot, new Set(['context-bridge.patch.yml', 'plugin']));
  ...
  const assets = [];
  let total = 0;
  for (const relative of CONTEXT_POC_SOURCE_FILES) {
    ...
    assets.push(Object.freeze({
      ...
      data,
      sha256: contextPocSha256(data, runtime.crypto || crypto)
    }));
  }
  const digestInput = assets.map((asset) => `${asset.relative}\0${asset.sha256}`).join('\0');
  return Object.freeze({
    assets: Object.freeze(assets),
    digest: contextPocSha256(digestInput, runtime.crypto || crypto)
  });
}
...
function contextPocVerifyTarget(root, inventory, expectedOwner, runtime = {}) {
  ...
  if (!markerStat.isFile() || markerStat.isSymbolicLink()
      || markerStat.size > 4096) throw new Error('context POC marker invalid');
  ...
  exactEntries(root, new Set([
    '.whaledock-context-poc.json', 'context-bridge.patch.yml', 'home'
  ]));
  let sealedRoot = pathImpl.join(root, 'home');
  contextPocAssertDirectory(sealedRoot, fsImpl);
  exactEntries(sealedRoot, new Set(['profiles']));
  sealedRoot = pathImpl.join(sealedRoot, 'profiles');
  contextPocAssertDirectory(sealedRoot, fsImpl);
  exactEntries(sealedRoot, new Set(['web']));
  sealedRoot = pathImpl.join(sealedRoot, 'web');
  contextPocAssertDirectory(sealedRoot, fsImpl);
  exactEntries(sealedRoot, new Set(['node_modules']));
  sealedRoot = pathImpl.join(sealedRoot, 'node_modules');
  contextPocAssertDirectory(sealedRoot, fsImpl);
  exactEntries(sealedRoot, new Set(['@whaledock']));
  sealedRoot = pathImpl.join(sealedRoot, '@whaledock');
  contextPocAssertDirectory(sealedRoot, fsImpl);
  exactEntries(sealedRoot, new Set(['context-bridge-poc']));
  ...
  for (const asset of inventory.assets) {
    ...
    if (contextPocSha256(data, runtime.crypto || crypto) !== asset.sha256) {
  ...
  for (const part of [
    'home', 'profiles', 'web', 'node_modules', '@whaledock', 'context-bridge-poc'
  ]) {
...
function contextPocOwnTemp(tempRoot, parent, fsImpl, pathImpl) {
  try {
    ...
    if (!relative || relative.startsWith('..') || pathImpl.isAbsolute(relative)
        || !pathImpl.basename(tempRoot).startsWith('.staging-')) return false;
    const marker = pathImpl.join(tempRoot, '.whaledock-context-poc-staging');
    ...
    return value && value.schema === 1 && value.package === CONTEXT_POC_PACKAGE
      && typeof value.digest === 'string' && CONTEXT_POC_TOKEN_RE.test(value.digest)
...
function contextPocOwnMount(mountRoot, parent, expectedOwner, fsImpl, pathImpl) {
  try {
    ...
    return marker && Object.keys(marker).sort().join(',') === 'digest,ownerToken,package,schema'
      ...
      && typeof marker.digest === 'string' && CONTEXT_POC_TOKEN_RE.test(marker.digest)
      && marker.ownerToken === expectedOwner
      && pathImpl.basename(mountRoot) === (
        `run-${marker.digest.slice(0, 16)}-${expectedOwner.slice(0, 16)}`
      );
  } catch (_error) {
    return false;
  }
...
function prepareContextPocAssets(plan, runtime = {}) {
  ...
  const fsImpl = runtime.fs || fs;
  const pathImpl = contextPocPathApi(plan.platform, runtime.pathModule);
  const inventory = { ...contextPocReadAssets(plan, runtime), platform: plan.platform };
  ...
  const parent = contextPocEnsureChildDirectory(versionRoot, 'runs', fsImpl, pathImpl);
  const randomBytes = runtime.randomBytes || crypto.randomBytes;
  const mountOwner = randomBytes(32).toString('hex');
  const targetRoot = pathImpl.join(
    parent, `run-${inventory.digest.slice(0, 16)}-${mountOwner.slice(0, 16)}`
  );
  if (fsImpl.existsSync(targetRoot)) throw new Error('context POC run id collision');
  const tempRoot = pathImpl.join(parent, `.staging-${inventory.digest}-${mountOwner.slice(0, 16)}`);
  ...
  return Object.freeze({
    ...
    mountParent: parent,
    mountOwner,
    homePath: pathImpl.join(targetRoot, 'home'),
...
function applyContextPocCommand(command, prepared) {
  ...
  args.splice(webIndexes[0] + 1, 0, '--patch', prepared.patchPath);
  return {
    ...command,
    args,
...
function start(config, hooks = {}, runtime = {}) {
  ...
  if (contextPlan.eligible) {
    ...
    try {
      ...
      prepared = preparer(contextPlan, runtime);
      command = applyContextPocCommand(resolvedCommand, prepared);
      contextBridgeReason = 'ready';
    } catch (_error) {
      if (prepared && prepared.mountRoot && prepared.mountParent) {
        try {
          cleanupContextPocMount(
            prepared.mountRoot, prepared.mountParent, prepared.mountOwner,
            { ...runtime, platform }
          );
        } catch (_cleanupError) { /* 回退原生启动优先，残留仍是 app-owned 隔离目录 */ }
      }
      // POC 资产或装饰失败时完整回退原 bundled command，原生聊天继续可用。
      command = resolvedCommand;
      contextBridgeReason = 'bridge-unavailable';
    }
  }
  const spawnImpl = runtime.spawn || spawn;
  const commandEnv = command.contextBridgeMounted === true
    ? contextPocEnv(baseEnv, command.env || {})
    : { ...baseEnv, ...(command.env || {}) };
  let child;
  try {
    child = spawnImpl(spawnFile(command, platform), command.args, {
    ...
    if (command.contextBridgeMounted === true) {
      cleanupContextPocMount(
        command.contextBridgeMountRoot,
        command.contextBridgeMountParent,
        command.contextBridgeMountOwner,
        { ...runtime, platform }
      );
    }
    throw error;
  ...
  let contextMountCleaned = false;
  const cleanupContextMount = () => {
    if (contextMountCleaned || command.contextBridgeMounted !== true) return;
    contextMountCleaned = true;
    try {
      cleanupContextPocMount(
        command.contextBridgeMountRoot,
        command.contextBridgeMountParent,
    ...
    } catch (_error) { /* POC 清理失败不覆盖真实进程退出事实 */ }
  };
