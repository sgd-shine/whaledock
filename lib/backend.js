...
const CONTEXT_POC_LIMITS = Object.freeze({
  ...
  // Host 的 console.read 可见结果上限是 64KiB；只放宽 settle
  // 的有界输出信封，其他端点仍按 8KiB fail-closed。
  maxWorkspaceSettleRequestBytes: 72 * 1024,
...
function contextBridgeRpc(state, endpoint, payload, binding, options = {}) {
  ...
  const maxRequestBytes = endpoint === 'workspace/files/settle'
    ? CONTEXT_POC_LIMITS.maxWorkspaceSettleRequestBytes
    : CONTEXT_POC_LIMITS.maxRequestBytes;
...
// 内容态只能把已选中会话的确定性匿名引用交给主进程。
// host 代际参与 HMAC，旧 host 的引用不能跨重连复用；raw id 始终只留在各自进程的内存映射中。
function dshDeliveryTargetRef(secret, hostInstanceId, raw) {
  if (typeof secret !== 'string' || !CONTEXT_POC_TOKEN_RE.test(secret)) {
    throw new TypeError('deliveryTargetSecret 必须是 64 位十六进制共享密钥');
  }
  if (typeof hostInstanceId !== 'string' || !CONTEXT_POC_ID_RE.test(hostInstanceId)) {
    throw new TypeError('deliveryTargetHostInstanceId 无效');
  }
  if (typeof raw !== 'string' || !raw) {
    throw new TypeError('delivery target raw id 无效');
  }
  const digest = crypto.createHmac('sha256', secret)
    .update(`whaledock-delivery-target-v1\0${hostInstanceId}\0${raw}`)
    .digest('hex');
  return `delivery-target-${digest}`;
}
...
function createDshPromptAdapter(options = {}) {
  ...
  const deliveryTargetSecret = options.deliveryTargetSecret;
  const deliveryTargetHostInstanceId = options.deliveryTargetHostInstanceId;
  const deliveryTargetConfigured = deliveryTargetSecret !== undefined
    || deliveryTargetHostInstanceId !== undefined;
  if (deliveryTargetConfigured) {
    try {
      // 构造期同时校验共享密钥与 host 代际，避免半配置降级为普通列表目标。
      dshDeliveryTargetRef(
        deliveryTargetSecret,
        deliveryTargetHostInstanceId,
        'configuration-proof'
      );
    } catch (_error) {
      throw promptContract('delivery target 精确解析配置无效');
    }
  }
  const baseHttp = `http://127.0.0.1:${port}`;
  ...
  function validateSessionRow(value, index) {
    ...
    return {
      ...
      cwd,
      blank: value.blank,
      exactSelectable: value.origin !== 'subagent'
  ...
  async function resolveTarget(deliveryTargetRef) {
    assertOpen();
    if (!deliveryTargetConfigured
        || typeof deliveryTargetRef !== 'string'
        || !/^delivery-target-[a-f0-9]{64}$/.test(deliveryTargetRef)) {
      throw dshPromptError('ERR_DSH_PROMPT_TARGET', '精确目标无效，请重新选择当前会话');
    }
    if (!packageProven) {
    ...
    const matches = rows.filter((row) => {
      if (!row.exactSelectable) return false;
      const candidate = dshDeliveryTargetRef(
        deliveryTargetSecret,
        deliveryTargetHostInstanceId,
        row.sessionId
      );
      return crypto.timingSafeEqual(
        ...
        Buffer.from(deliveryTargetRef, 'utf8')
      );
    });
    if (matches.length !== 1) {
      throw dshPromptError('ERR_DSH_PROMPT_TARGET', '精确目标已失效，请重新选择当前会话');
    }
    // 同一 adapter 最多保留一个精确解析条目；普通 cockpit 列表 token 不受影响。
    for (const [token, entry] of rawByTargetToken) {
      if (entry.allowBlankExact === true) rawByTargetToken.delete(token);
    }
    const occupiedTokens = new Set(rawByTargetToken.keys());
    const token = targetToken(occupiedTokens);
    const row = matches[0];
    const workspaceLabel = row.cwd
      ? (path.win32.isAbsolute(row.cwd) ? path.win32.basename(row.cwd) : path.basename(row.cwd))
      : '';
    const entry = {
      rawSessionId: row.sessionId,
      label: workspaceLabel || '当前会话',
      running: row.running,
      updatedAt: row.updatedAt,
      cwd: row.cwd,
      allowBlankExact: true
    };
    rawByTargetToken.set(token, entry);
    return inspectedTarget(token, entry);
  }
  ...
  async function revalidateTarget(targetTokenValue) {
    ...
    if (!row || !(row.selectable
        || (entry.allowBlankExact === true && row.exactSelectable))) {
