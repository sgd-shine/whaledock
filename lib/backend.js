...
function resolveCommand(config) {
  if (config.command && String(config.command).trim()) {
    return { file: String(config.command).trim(), args: [], shell: true, label: String(config.command).trim() };
  }
  const dsh = which('dsh');
  if (dsh) return { file: dsh, args: ['web'], shell: false, label: 'dsh web' };
  const npx = which('npx');
  if (npx) return { file: npx, args: ['-y', '@deepseek-ai/dsh', 'web'], shell: false, label: 'npx @deepseek-ai/dsh web' };
  return null;
}
...
function start(config, hooks = {}) {
  ...
  const state = { child, label: cmd.label, exited: false, code: null };
