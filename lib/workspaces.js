...
// 受保护根与路径身份口径只在 lib/config.js 声明一次。
...
function canonicalWorkspace(value, options = {}) {
  ...
  let real;
  try { real = nativeRealpathSync(fsImpl)(resolved); } catch (error) {
    throw workspaceError('ERR_WORKSPACE_REALPATH', '无法验证工作区真实路径', error);
  }
  const canonicalPath = normalizeRealPath(real, { platform, pathImpl: tools });
...
function ensureDefaultWorkspace(documentsDir, options = {}) {
  ...
  try {
    effectiveDocuments = normalizeRealPath(nativeRealpathSync(fsImpl)(documentsPath),
...
function createWorkspaceSwitchCoordinator(options = {}) {
  ...
  const beforeSwitch = typeof options.beforeSwitch === 'function'
    ? options.beforeSwitch : async () => {};
  ...
  const switchOnce = async (targetValue) => {
    ...
    // 目标已规范化、非 noop 且预算允许后才做有副作用的连接复核。
    // 复核与事务共用同一 serial，避免并发请求在 preflight await 期间反转顺序。
    await beforeSwitch(target.path);
...
module.exports = {
  ...
  MAX_RECENT_WORKDIRS,
  // v0.6 工作台包在建目录之前要先单独做一轮字面校验（目录还不存在，canonicalWorkspace 用不了）。
  assertWorkspaceNotForbidden,
  canonicalWorkspace,
