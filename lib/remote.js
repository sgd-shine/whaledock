...
function normalizeInbound(value) {
  ...
  onlyKeys(value, ['actorId', 'kind', 'content', 'sourceId', 'receivedAt'], '收件');
  const actorId = fixedString(value.actorId, '发送者标识', 256);
  const sourceId = value.sourceId === undefined
    ? null : fixedString(value.sourceId, '平台来源标识', 512);
  const receivedAt = value.receivedAt === undefined
    ? null : fixedString(value.receivedAt, '平台收件时间', 64);
  if (receivedAt !== null) {
    const parsed = new Date(receivedAt);
    if (!Number.isFinite(parsed.getTime()) || parsed.toISOString() !== receivedAt) {
      throw remoteError('平台收件时间无效');
    }
  }
  const sourceBytes = sourceId ? Buffer.byteLength(sourceId, 'utf8') : 0;
  const receivedAtBytes = receivedAt ? Buffer.byteLength(receivedAt, 'utf8') : 0;
  ...
  if (value.kind === 'file') {
    ...
    return Object.freeze({
      ...
      retainedBytes: Buffer.byteLength(name, 'utf8')
        + Buffer.byteLength(reference, 'utf8') + sourceBytes + receivedAtBytes
    });
  }
  ...
  return Object.freeze({
    actorId, kind: value.kind, sourceId, receivedAt, content,
    retainedBytes: retainedBytes + sourceBytes + receivedAtBytes
  });
}
...
function createRemoteService(options = {}) {
  ...
  async function disconnectTransport(entry, reason, capturedTargets = null) {
    const lease = capturedTargets ? capturedTargets.lease : entry.sessionLease;
    if (lease) {
      await disconnectSession(lease, lease.session, reason);
      if (entry.sessionLease === lease) {
        entry.session = null;
        entry.sessionLease = null;
      }
    }
    const attempt = capturedTargets ? capturedTargets.attempt : entry.connectAttempt;
    if (attempt) {
  ...
  function adapterOperation(entry, method, value, guard = null) {
    ...
    const lease = entry.sessionLease;
    ...
    return deadline(
      async () => {
        if (entry.sessionLease !== lease || generation !== entry.connectionGeneration
  ...
  function adapterLifecycle(id, generation, state, _reasonCode) {
    const entry = entryFor(id);
    if (generation !== entry.connectionGeneration) return Promise.resolve(staleResult());
    // adapter 输入属于平台边界：即使字符形式像 reasonCode，也可能是
    // token/secret。公开快照与审计只写核心自有的固定枚举。
    const nextReason = state === 'error' ? 'transport-error' : 'remote-closed';
    // 先把旧代回收占位放进本通道队列，再触发 abort/审计/UI 回调。
    // 这些回调可能同步请求重连；重连必须排在旧 session 确认断开之后。
    const capturedTargets = Object.freeze({
      lease: entry.sessionLease,
      attempt: entry.connectAttempt
    });
    const cleanup = enqueueEntry(entry, async () => {
      try { await disconnectTransport(entry, nextReason, capturedTargets); }
      catch (_error) { recordDisconnectFailure(entry); }
      return { accepted: true };
    }, true);
    // 使 lifecycle 事件前已排队但尚未执行的 enable 意图失效；
    // 状态回调在此之后新建的意图会拿到新 version，并排在 cleanup 之后。
  ...
  async function connectEntry(entry, lifecycleVersion) {
    ...
    if (entry.sessionLease) {
      // 上一代断开超时/拒绝时 session 仍受 entry 持有。未确认回收前
      // 禁止新 connect 覆盖句柄，否则 UI 会假绿且旧连接永久不可达。
      try { await disconnectTransport(entry, 'reconnect-cleanup'); }
      catch (_error) {
        recordDisconnectFailure(entry);
        return;
      }
      if (!entry.enabled || lifecycleVersion !== entry.lifecycleVersion || closing) return;
    }
    if (!entry.adapter) {
  ...
  async function trustedSensitivity(entry, operation, kind, classificationContext) {
    try {
      const request = { channelId: entry.id, operation, kind };
      // 只有主进程自己持有的对象身份可作为本地上下文；平台正文仍不能
      // 自报 sensitivity。是否信任该上下文，最终仍由 classifyContent 决定。
      if (classificationContext !== undefined) {
        request.classificationContext = classificationContext;
      }
      const value = await deadline(
        () => classifyContent(Object.freeze(request)),
        {
      ...
      try {
        await deadline(
          () => receiveSink(Object.freeze({
            channelId: entry.id,
            kind: inbound.kind,
            ...(inbound.sourceId ? { sourceId: inbound.sourceId } : {}),
            ...(inbound.receivedAt ? { receivedAt: inbound.receivedAt } : {}),
  ...
  function applyLateApprovalResult(entry, request, result) {
    const current = approvalState(request.requestToken);
    // 迟到的旧 receipt 只能把未知提升为已知，不得覆盖后续权威
    // 回读得到的确定决定，也不得重复增加 approved 计数。
    if (current === 'adopt' || current === 'redo') return;
  ...
  async function pushToEntry(entry, item, classificationContext) {
    ...
    const classified = await trustedSensitivity(
      entry, 'push', item.kind, classificationContext
    );
    if (!classified.ok) {
  ...
  // 设置页的通道自检必须精确投到目标平台；不能复用广播 push，避免未来
  // 钉钉/随身网页也在线时，一次飞书测试被复制到其他通道。
  function pushTo(id, value, options = {}) {
    let entry;
    let item;
    try {
      assertUsable();
      entry = entryFor(id);
      item = normalizePush(value);
      if (!isRecord(options)) throw remoteError('目标推送参数必须是对象');
      onlyKeys(options, ['classificationContext'], '目标推送参数');
    } catch (error) { return Promise.reject(error); }
    const retainedBytes = Buffer.byteLength(item.body, 'utf8')
      + (item.dedupeKey ? Buffer.byteLength(item.dedupeKey, 'utf8') : 0);
    return enqueueOutbound(entry, retainedBytes, () => (
      pushToEntry(entry, item, options.classificationContext)
    )).then((result) => {
      changed();
      return {
        delivered: result.delivered ? [result.channelId] : [],
  ...
  function close() {
    ...
    closePromise = Promise.allSettled(operations).then((results) => {
      ...
      // 显式拒绝的 disconnect 可能是瞬时故障；保留 closing 隔离，但允许
      // 后续 close 重试同一代。超时的 raw receipt 仍由 entry 缓存，不重发。
      closePromise = null;
      throw error;
    });
    return closePromise;
