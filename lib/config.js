...
const PREVIOUS_DEFAULT_DSH_VERSION = '0.1.0-rc.6';
...
const DEFAULTS = {
  ...
  // v0.6 叫醒阶梯（C-4）。每一层都能单独关，另有一个「安静模式」总开关只留托盘图标。
  // 30 秒那一档默认开：这是整个 v0.6 里最能拉开与 Web 内嵌桌宠差距的一秒钟。
  // 提示音默认关：声音最容易招人烦，必须用户主动打开。
  quietMode: false,
  wakeNudgeEnabled: true,
  wakeEscalateEnabled: true,
  wakeSoundEnabled: false,
  // v0.6 工作台包：null = 默认工作台（不切工作台，一切跟随全局设置）。
  workbenchId: null,
  // ⌘⇧0 回到上一个用过的工作台，由 main 维护，设置页不直写。
  lastWorkbenchId: null,
  // 首次引导与重工作台确认都要跨 App 重启记住；这是内部状态，不开放给设置页。
  // 包被用户移除时 main 会清理对应 id，让同名包以后重装时重新走首次确认。
  workbenchOnboardingSeenIds: [],
  workbenchHeavyConfirmedIds: [],
  // 旧 workdir/workbenchId -> 项目注册表只迁移一次；不清理旧字段，且不向设置页开放。
  projectMigrationVersion: 0,
  // v0.10 原生内容区视图偏好；提示是否看过属于内部状态，不开放给设置页。
  contentViewMode: 'content',
  contentViewHintSeen: false,
  // v0.7 远程板块。三个通道默认全关；开启偏好与真实连接状态分开，
  // 设置页绝不能因为用户勾了开关就把尚未配置的平台画成在线。
  remoteFeishuEnabled: false,
  remoteDingtalkEnabled: false,
  remoteWebEnabled: false,
  // 允许走第三方 IM 的三类默认推送。通道本身默认关闭，所以升级后零联网。
  remoteImApprovals: true,
  remoteImTaskCompletions: true,
  remoteImReports: true
};
...
function normalizeWorkbenchId(key, value, nullable = true) {
  if (value == null || String(value).trim() === '') {
    if (nullable) return null;
    throw validationError(key, `${key} 只能包含合法工作台标识`);
  }
  const text = String(value).trim();
  if (text.length > 96 || /[\u0000-\u001f\u007f\\/]/.test(text)
      || !/^(?:builtin|user):/.test(text)) {
...
function normalizeValue(key, value, options = {}) {
  ...
  if (key === 'contentViewMode') {
    if (value !== 'content' && value !== 'sessions') {
      throw validationError(key, 'contentViewMode 必须是 content 或 sessions');
    }
    return value;
  }
  if (key === 'projectMigrationVersion') {
    if (value !== 0 && value !== 1) {
      throw validationError(key, 'projectMigrationVersion 只能是 0 或 1');
    }
    return value;
  }
  // 两份首次确认记忆与扫描上限同为 64。精确保留 Unicode 码点，不做 NFC/NFD 归一化；
  // 非法旧值由 normalizeStored 单字段回落为空数组，最多只会让引导再弹一次。
  if (key === 'workbenchOnboardingSeenIds' || key === 'workbenchHeavyConfirmedIds') {
    if (!Array.isArray(value) || value.length > 64) {
      throw validationError(key, `${key} 必须是不超过 64 项的工作台标识数组`);
    }
    const result = [];
    for (const entry of value) {
      const id = normalizeWorkbenchId(key, entry, false);
      if (!result.includes(id)) result.push(id);
    }
    return result;
  }
  // 工作台 id 同样只是选择标识；真正的路径限界由 lib/workbenches.js 负责。
  // 与宠物/主题不同的是它允许 null——那表示「默认工作台」，即不启用任何包。
  if (key === 'workbenchId' || key === 'lastWorkbenchId') {
    return normalizeWorkbenchId(key, value, true);
  }
...
function init(baseDir, options = {}) {
  ...
  // rc.6 是 v0.2–v0.7 写入磁盘的旧默认值；只迁移字节级精确命中，
  // 用户选的 latest、其他版本或非规范空白表达都不代改。
  const migrateDshDefault = raw.dshVersion === PREVIOUS_DEFAULT_DSH_VERSION;
  if (migrateDshDefault) normalized.dshVersion = DSH_CONTRACT.packageVersion;
  data = { ...clone(DEFAULTS), ...normalized };
  if (migrateDshDefault) persist({ ...raw, dshVersion: DSH_CONTRACT.packageVersion });
  return clone(data);
}
