...
## v0.7 源码预览：视频驾驶舱色系（尚未发布）

视频驾驶舱顶栏可直接选择七套内置色系：鲸坞深色、鲸坞浅色、极光、墨鲸、日落珊瑚、竹影青黛、潮汐靛蓝；放进主题文件夹的用户主题也会出现在同一个下拉中，旁边「＋」可直达自定义入口。

这里切换的是 **WhaleDock 全局主题**：会同步刷新并持久化到鲸坞自己的主窗、设置、看板等界面，退出驾驶舱或重启后仍保留。工作台包若自带 `theme.json`，顶栏会锁定为包主题。完整 dsh/Harness 对话内容始终不注入 CSS 或 DOM，不会跟着换肤。

本地源码态 `npm run smoke` 为 **451 PASS / ALL PASS**，macOS 隔离源码 App 已完成多色系点击、重启持久化和 `960×620` / `1280×820` 响应式回读。公开稳定版仍为 v0.6.0；没有 v0.7 安装包、tag 或 Release。
...
- **v0.6.0**：工作台包机制、内置短视频创作台、托盘五态与叫醒阶梯，已实现、过三平台 CI 并公开发布；macOS 成品已正式签名并通过 Apple 公证，是当前稳定下载版。社区包目录、`.whaledock` 一键装（要专门处理 zip-slip）、提示词输入表单与批量执行明确保留为 v0.7；`agent.cordis.yml` 本版**只检测不接通**，等 dsh 上游侦察确认 preset 加载方式后再做。
