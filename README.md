...
当前稳定下载版是 [`v0.6.0`](https://github.com/sgd-shine/whaledock/releases/tag/v0.6.0)。
...
本地统一 `npm run smoke` 为 **315 PASS / ALL PASS**，正式 tag 提交的三平台 CI 全绿。
...
v0.5 给鲸坞加了两件可以完全自定义的东西，**都不需要写代码**。v0.5 给鲸坞加了两件可以完全自定义的东西，**都不需要写代码**。
...
本地统一 `npm run smoke` 为 **233 PASS / ALL PASS**，正式 tag 提交的三平台 CI 全绿，八项 Release 资产已回读。但**宠物动效、透明背景、鼠标穿透与四套主题的实际配色都还没有人工目视验收**，只有代码与启动日志证据；人工体验项见 [第二阶段总验收清单](docs/第二阶段总验收清单-2026-08-16.md)，完整遗留见 [遗留清单](docs/遗留清单-2026-08-16.md)。Windows 与 Intel Mac 全线未真机；macOS 自 v0.5.1 起为 ad-hoc 签名，仍未公证。
...
从 v0.5.1 起，macOS 安装包带 **ad-hoc 代码签名**（无证书、无费用），但**仍未做 Apple 公证**。

正常路径（v0.5.1 及以后）：

1. 打开 dmg，把 WhaleDock 拖进「应用程序」；
2. 双击打开，提示「无法验证开发者」时点「完成」；
3. 打开 **系统设置 → 隐私与安全性**，拉到底部点 **「仍要打开」**，再确认一次；
4. 之后正常双击即可。

> macOS 15 (Sequoia) 起，Apple 移除了「右键 → 打开」这条绕行入口，请按上面的系统设置路径操作。

**如果提示「文件已损坏，无法打开，你应该将它移到废纸篓」**：这不是文件下载坏了，而是
**v0.5.0 及更早版本的安装包完全没有代码签名**——Apple Silicon 要求所有原生代码带有效签名，
包一旦被下载打上 `com.apple.quarantine` 隔离属性，Gatekeeper 校验包级签名失败就会报「已损坏」，
而且这条提示**用右键「打开」救不回来**。两种解决办法：

- 推荐：改下载 **v0.5.1 或更新版本**；
- 或者对已装好的旧版执行一次（只需一次，不影响其他软件）：
...
- **v0.6.0**：工作台包机制、内置短视频创作台、托盘五态与叫醒阶梯，已实现、过三平台 CI 并公开发布，是当前稳定下载版。社区包目录、`.whaledock` 一键装（要专门处理 zip-slip）、提示词输入表单与批量执行明确保留为 v0.7；`agent.cordis.yml` 本版**只检测不接通**，等 dsh 上游侦察确认 preset 加载方式后再做。
- **v1.0**：签名与公证需要付费开发者账号与证书，属于要 SGD 决定的事项，当前明确不做。
